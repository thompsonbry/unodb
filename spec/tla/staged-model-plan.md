...
### H9: Upsert-erase CAS gap (positioned erase)
- Upserter observes value V under RCS, lambda returns "erase"
- Upserter releases RCS, re-traverses to acquire write lock
- Concurrent writer changes value from V to V′ in the gap
- Upserter must detect version mismatch and retry (not erase V′)
- **Property violated:** CAS atomicity (erasing value lambda didn't observe)
...
**File:** `OLCUpsertErase.tla`
**Hazard:** H9 (upsert lambda observes value, returns erase, value changes in gap)
**What:** 1 upserter + 1 concurrent writer. Upserter observes value under RCS,
releases, re-traverses, validates version before erasing. Writer can change
the value in the gap. Proves the version check is necessary and sufficient
for CAS semantics.
**Variables (7):**
- Shared: `value`, `version`
- Upserter: `upc`, `u_observed`, `u_obs_ver`
- Writer: `wpc`, `w_new_val`

**Domain:** Values = {V1, V2}, version ∈ 0..10
**Result:** 1,878 distinct states, <1s. All properties pass.
**Properties:**
- `CASSafety`: if upserter holds write lock, value == what lambda observed
- `MutualExclusion`: upserter and writer never both hold write lock
- `VersionConsistency`: version parity tracks lock ownership
- `NoEraseOfEmpty`: never erases an already-empty slot
- `UpsertProgress` (temporal): upserter eventually completes

**Bug demo:** `OLCUpsertErase_BugDemo.tla` — removes version check, CASSafety
violated at depth 8 (upserter erases V2 after observing V1).
...
| 9 ✓ | H9 | 1 | 1W+1U | 1,878 | <1s | CAS safety |
...
├── OLCDoubleCut.cfg
├── OLCUpsertErase.tla            # Stage 9 (DONE)
├── OLCUpsertErase.cfg
├── OLCUpsertErase_BugDemo.tla    # Stage 9 bug demo
├── OLCUpsertErase_BugDemo.cfg
