 ...
 protected:
  ...
  class it_t {   
   public:
    ...
    // Position the iterator and the end of the index.
    //
    // @return true
    bool end() noexcept {
      invalidate();
  ...
  // Return an iterator for the tree which is not positioned on any leaf.
  //
  // FIXME Replace with begin() and end() iterators methods that are
