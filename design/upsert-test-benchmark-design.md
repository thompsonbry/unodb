...
| 13 | `key_view` | `value_view` | Variable key, explicit leaf | `can_eliminate_leaf = false`, update blocked by static_assert |
| 13b | `uint64_t` | `value_view` | Fixed key, explicit leaf | Same constraint |

For types 13/13b: verify `update` action is a compile error (static_assert).
Test `keep` and `erase` only.
