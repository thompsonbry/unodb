...
upgrade — minimizes write-lock hold time. The trade-off is documented
...
for this specific API where the lambda _resolves_ what to do with the
...
**Finding:** The existing `try_remove` validates its _own_ read version (from
the re-traversal), not the upserter's captured observation version. The CAS
contract requires validating that the node hasn't changed since the _upserter's
observation_, not just since the re-traversal's read.
...
**db / mutex_db:** Direct implementation (no try\_ prefix, no retry loop).
